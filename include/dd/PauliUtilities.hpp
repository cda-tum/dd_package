...
void bitsetCopySegment(std::bitset<N>& x, const std::bitset<M> y, unsigned int begin_x, unsigned int begin_y, unsigned int end_y) {
...
phase_t getPhaseInverse(phase_t a) {
//    	int A = (int) a;
//    	A = (A & 0x1) | ( ((A&1) << 1) ^ (A&2) );
//    	return (phase_t) A;
...
phase_t getPhaseaMinusB(phase_t a, phase_t b) {
	phase_t minb = getPhaseInverse(b);
	return (phase_t) ((a + minb) & 0x3);
}

std::string phaseToString(phase_t a) {
	switch(a) {
...
bool stabilizerGroupsEqual(const StabilizerGroup& G, const StabilizerGroup& H) {
...
bool stabilizerGroupIsSorted(const StabilizerGroup& G) {
...
void printStabilizerGroup(const StabilizerGroup& G) {
...
void printStabilizerGroup(const StabilizerGroup& G, unsigned int nQubits) {
	Log::log << "Stabilizer group (" << G.size() << " elements) = {";
	for (unsigned int i=0; i<G.size(); i++) {
...
void printStabilizerGroup(const std::vector<LimBitset<NUM_QUBITS>*>& G) {
	if (G.size() == 0) {
		Log::log << "  (empty group)\n";
	}
	for (unsigned int i=0; i<G.size(); i++) {
...
template <std::size_t NUM_QUBITS>
void printKernel(const std::vector<std::bitset<NUM_QUBITS> >& kernel) {
...
std::string groupToString(const StabilizerGroup& G, Qubit nQubits) {
	std::stringstream ss;
...
StabilizerGroup groupConcatenate(const StabilizerGroup& G, const StabilizerGroup& H) {
...
template <std::size_t NUM_QUBITS>
std::vector<LimBitset<NUM_QUBITS>*> appendIdentityMatrixBitset(const std::vector<LimEntry<NUM_QUBITS>*>& G) {
...
template <std::size_t NUM_QUBITS>
void pruneZeroColumns(std::vector<LimEntry<NUM_QUBITS>*>& G) {
	unsigned int i=0;
	while (i<G.size()) {
...
template <std::size_t NUM_QUBITS>
void pruneZeroColumns(std::vector<LimBitset<NUM_QUBITS>*>& G) {
	unsigned int i=0;
	while (i<G.size()) {
...
template <std::size_t NUM_QUBITS>
void pruneZeroColumnsModuloPhase(std::vector<LimBitset<NUM_QUBITS>*>& G) {
	unsigned int i=0;
	while (i<G.size()) {
