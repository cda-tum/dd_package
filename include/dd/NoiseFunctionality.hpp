...
namespace dd {
    ...
    public:
        StochasticNoiseFunctionality(const std::unique_ptr<customPackage>&   cLocalDD,
                                     ...
                                     size_t                                  cSeed):
            localDD(cLocalDD),
            seed(cSeed), dist(0.0, 1.0L), generator(cSeed) {
            nqubits = size;
        ...
        inline void applyNoiseOperation(const std::vector<dd::Qubit>& usedQubits, dd::mEdge dd_op, dd::vEdge& localRootEdge) {
            bool multiQubitOperation = usedQubits.size() > 1;

            for (auto& target: usedQubits) {
                auto operation = generateNoiseOperation(dd_op, target, false, multiQubitOperation);
                auto tmp       = localDD->multiply(operation, localRootEdge);

                if (dd::ComplexNumbers::mag2(tmp.w) < dist(generator)) {
                    operation = generateNoiseOperation(dd_op, target, true, multiQubitOperation);
        ...
        dd::mEdge generateNoiseOperation(dd::mEdge   dd_operation,
            ...
            std::uint_fast8_t effect;
            for (const auto& noise_type: gateNoiseEffects) {
                ...
                switch (effect) {
                    ...
                    case (qc::ATrue): {
                        ...
                        if (tmp_op.p == nullptr) {
                            tmp_op = localDD->makeGateDD(multiQubitOperation ? ampDampingTrueMulti : ampDampingTrue, nqubits, target);
                    ...
                    case (qc::AFalse): {
                        ...
                        if (tmp_op.p == nullptr) {
                            tmp_op = localDD->makeGateDD(multiQubitOperation ? ampDampingFalseMulti : ampDampingFalse, nqubits, target);
                    ...
                    case (qc::X): {
                        ...
                        if (tmp_op.p == nullptr) {
                            tmp_op = localDD->makeGateDD(dd::Xmat, nqubits, target);
                    ...
                    case (qc::Y): {
                        auto tmp_op = localDD->stochasticNoiseOperationCache.lookup(effect, target);
                        if (tmp_op.p == nullptr) {
                            tmp_op = localDD->makeGateDD(dd::Ymat, nqubits, target);
                            localDD->stochasticNoiseOperationCache.insert(effect, target, tmp_op);
                        }
                        dd_operation = localDD->multiply(tmp_op, dd_operation);
                        break;
                    ...
                    case (qc::Z): {
                        auto tmp_op = localDD->stochasticNoiseOperationCache.lookup(effect, target);
                        if (tmp_op.p == nullptr) {
                            tmp_op = localDD->makeGateDD(dd::Zmat, nqubits, target);
                            localDD->stochasticNoiseOperationCache.insert(effect, target, tmp_op);
                        }
                        dd_operation = localDD->multiply(tmp_op, dd_operation);
                        break;
        ...
        [[nodiscard]] std::uint_fast8_t returnNoiseOperation(dd::noiseOperations noiseOperation, double prob, bool multi_qubit_noise) const {
