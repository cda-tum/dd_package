...
namespace dd {
   ...
   public:
       ...
       template<class Node>
       Edge<Node> makeDDNode(Qubit var, const std::array<Edge<Node>, std::tuple_size_v<decltype(Node::e)>>& edges, bool cached = false, const bool firstPathEdge = true) {
   ...
   public:
       template<class Node>
       Edge<Node> add2(const Edge<Node>& x, const Edge<Node>& y, bool firstPathEdge = true) {
           ...
           auto  r            = computeTable.lookup({x.p, x.w}, {y.p, y.w}, firstPathEdge);
...
//           }
           ...
           computeTable.insert({x.p, x.w}, {y.p, y.w}, {e.p, e.w}, firstPathEdge);
           return e;
       }
                   ...
                   for (auto k = 0U; k < ROWS; k++) {
                       ...
                       if constexpr (std::is_same_v<LeftOperandNode, dNode>) {
                           ...
                           if (firstPathEdge || idx == 1) {
                               // When firstPathEdge is true or I have the first edge I don't optimize anything and set firstPathEdge for all child edges
                           ...
                           if (k == 0 || edge[idx].w == Complex::zero) {
                               ...
                               auto old_e = edge[idx];
                               edge[idx] = add2(edge[idx], m, firstPathEdge);
...
namespace std {
   ...
   struct hash<dd::dEdge> {
       std::size_t operator()(dd::dEdge const& e) const noexcept {
           ...
           auto h3  = std::hash<short int>{}(e.p->flags & (7U));
//           auto h3  = std::hash<short int>{}(e.p->flags); //todo this can probably be relaxed to generate more hits
